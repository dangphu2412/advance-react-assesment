...
export default function Test1Solution2() {
    ...
    console.log("Render Test1Solution2");

    const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
        e.preventDefault();
        submit({username: userNameRef.current, password: passwordRef.current});
    }

    return <form onSubmit={handleSubmit} className={'w-[600px] grid grid-cols-2 gap-4 border border-gray-300 p-4 mx-auto mt-12'}>
        <Header>
            This is test1
        </Header>
        ...
        <input className={'border border-solid'} id={'username'} name={'username'} placeholder={'Type username'} onChange={(e) => userNameRef.current = e.target.value} />

        <label htmlFor={'password'}>
            Password
        </label>
        <input className={'border border-solid'} id={'password'} name={'password'} type={'password'} onChange={(e) => passwordRef.current = e.target.value} />
        ...
        <q className={'col-span-2'}>
            ...
            {/* Giải pháp: sử dụng preventDefault để ngăn chặn hành vi mặc định của form khi submit */}
            {/* Lý do trang bị reload: khi submit form, browser sẽ load lại trang và gửi request đến server */}
        </q>
    </form>
}
