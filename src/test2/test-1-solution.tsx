...
export default function Test2() {
    ...
    function handleCreate(data: { name: string; description: string; tags: string[] }) {
        setCards([...cards, { id: cards.length + 1, ...data }]);
    }
            ...
            {cards.map((card, index) => {
                return <div className={'border border-black rounded p-4 flex flex-col gap-2'} key={index}>
                    <div className={'flex justify-between'}>
                        {card.name}
                        <span className={'border rounded w-8 text-center cursor-pointer'} onClick={() => handleDelete(index)}>
                            -
                        </span>
                    </div>
