...
export default function Test1Solution2() {
    ...
    const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
        e.preventDefault();
        console.log("Submit", (e.target as HTMLFormElement).username.value, (e.target as HTMLFormElement).password.value);
    }
    ...
    return <form onSubmit={handleSubmit} className={'w-[600px] grid grid-cols-2 gap-4 border border-gray-300 p-4 mx-auto mt-12'}>
        ...
        <input className={'border border-solid'} id={'username'} name={'username'} placeholder={'Type username'} />

        <label htmlFor={'password'}>
            Password
        </label>
        <input className={'border border-solid'} id={'password'} name={'password'} type={'password'} />
